...
import {
  AddProductFormDataSchema,
  UpdateProductFormDataSchema,
} from "@/schemas/server-action/product.schema";
...
export async function updateProduct(prevState: any, formData: FormData) {
  ...
  try {
    ...
    const validateData = validateFormDataWithZod(
      formData,
      UpdateProductFormDataSchema
    );
    ...
    const { error } = await withApiHandling(async () =>
      axios.put(`${urlConfig.KKU_API_URL}/products/${id}`, payload, {
...
export async function addProduct(prevState: any, formData: FormData) {
  ...
  try {
    ...
    const { error } = await withApiHandling(async () =>
      axios.post(`${urlConfig.KKU_API_URL}/products`, payload, {
        headers: buildHeaders({ token: user.token, uploadHeaders: true }),
      })
    );
    //"ไม่สามารถทำการบันทึกได้ เนื่องจากชื่อหรือรหัส มีอยู่ในระบบแล้ว"
...
//     async () =>
//       axios.get(
//         `${urlConfig.KKU_API_URL}/products/branch/${user.branchId}/unstocked-products`,
//         { headers: buildHeaders({ token: user.token }) }
//       ),
//     { option: { validateResponse: fetchProductsResSchema } }
//   );
