...
import { fetchCustomerGroupsResSchema, fetchCustomersResSchema } from "@/schemas/api/customer";
import { AddCustomerFormDataSchema, UpdateCustomerFormDataSchema } from "@/schemas/server-action/customer.schema";
import { withApiHandling } from "@/utils/api.utils";
import { buildHeaders } from "@/utils/httpHeaders";
import { renderFail, renderSuccess } from "@/utils/server-action-render.utils";
import { getSession } from "@/utils/session.utils";
import { validateFormDataWithZod, validateResponseFromServer } from "@/utils/validate.utils";
...
export type CustomerType = Awaited<ReturnType<typeof fetchCustomers>>[number];
...
export type CustomerGroupType = Awaited<ReturnType<typeof fetchCustomerGroups>>[number];
...
export async function updateCustomerAction(prevState: any, formData: FormData)
{
    ...
    try
    {
        ...
        const user = (await getSession()).user;
        const data = validateFormDataWithZod(formData, UpdateCustomerFormDataSchema);
        pathname = data.pathname ?? "/customer";

        const { id, ...payload } = data;

        await withApiHandling(() =>
            axios.put(`${urlConfig.KKU_API_URL}/customers/${id}`,
                payload,
                { headers: buildHeaders({ token: user.token }) }
            )
        );
...
export async function addCustomerAction(prevState: any, formData: FormData)
{
    try
    {
        ...
        const validate = validateFormDataWithZod(formData, AddCustomerFormDataSchema);
        ...
        const { error } = await withApiHandling(() =>
            axios.post(`${urlConfig.KKU_API_URL}/customers`,
                payload,
                { headers: buildHeaders({ token: user.token }) }
            ),
        );
