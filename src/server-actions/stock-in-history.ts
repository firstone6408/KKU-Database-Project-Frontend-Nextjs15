...
import { AddStockInHistoryFormDataSchema, FirstAddStockInHistoryFormDataSchema } from "@/schemas/server-action/stock.schema";
...
export async function fetchStockInHistories()
{
    ...
    const { error, result } = await withApiHandling(async () =>
        axios.get(`${urlConfig.KKU_API_URL}/stocks/branch/${user.branchId}/stock-in-history`,
...
export type StockInHistoryType = Awaited<ReturnType<typeof fetchStockInHistories>>[number];

export async function addStockInHistoryAction(prevState: any, formData: FormData)
{
    let _pathname = "/stock";
    try
    {
        ...
        const data = validateFormDataWithZod(formData, AddStockInHistoryFormDataSchema);
        ...
        const { error } = await withApiHandling(async () =>
            axios.post(`${urlConfig.KKU_API_URL}/stocks/add`,
                payload,
                { headers: buildHeaders({ token: user.token }) }
            )
        );
...
export async function firstAddStockInHistoryAction(prevState: any, formData: FormData)
{
    let _pathname = "/stock";
    try
    {
        ...
        const data = validateFormDataWithZod(formData, FirstAddStockInHistoryFormDataSchema);
        ...
        const { error: error1 } = await withApiHandling(async () =>
            axios.post(`${urlConfig.KKU_API_URL}/product-sale-branches/add`,
