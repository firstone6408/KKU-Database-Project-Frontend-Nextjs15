...
import { AddOrUpdatePaymentMethodFormDataSchema, RemovePaymentMethodFormDataSchema } from "@/schemas/server-action/payment-method.schema";
...
export type PaymentMethodType = Awaited<ReturnType<typeof fetchPaymentMethods>>[number];

export async function addOrUpdatePaymentMethodAction(prevState: any, formData: FormData)
{
    ...
    try
    {
        ...
        const data = validateFormDataWithZod(formData, AddOrUpdatePaymentMethodFormDataSchema);
        ...
        if (id !== "")
        {
            _axios = axios.put(`${urlConfig.KKU_API_URL}/payment-methods/${id}`,
                payload,
                { headers: buildHeaders({ token: user.token }) }
            )
        }
        ...
        {
            _axios = axios.post(`${urlConfig.KKU_API_URL}/payment-methods`,
                payload,
                { headers: buildHeaders({ token: user.token }) }
            )
        }
...
export async function removePaymentMethodAction(prevState: any, formData: FormData)
{
    let _pathname = "/payment-method";
    try
    {
        ...
        const { id, pathname } = validateFormDataWithZod(formData, RemovePaymentMethodFormDataSchema);
